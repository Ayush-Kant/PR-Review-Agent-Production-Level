...
The backend is a modular monolith with **inward-only** dependency direction. Boundary/infrastructure modules implement interfaces owned by inner/domain modules; inner modules do not depend directly on concrete GitHub, queue, database, provider, model, or cloud adapters.

No module may bypass an explicit boundary to use another module's database connection, queue client, provider SDK, GitHub client, authorization implementation, or model client.

## 2. Trust and policy are separate concerns
...
## 4. Review generation is monotonic
...
## 5. Retrieval evidence is snapshot-bound
...
## 6. Evidence and publication authority

Every canonical finding requires attributable change evidence, review/pass identity, provenance, retrieval freshness when repository evidence is used, and a policy decision before publication eligibility.
...
## 7. Publication is reconciliation-driven
...
## 8. Policy evaluation fails closed
...
## 9. Budget uncertainty fails closed
...
## 10. HITL authorization is capability-based
...
## 11. Evaluator isolation is directional
...
## 13. Privacy is minimization-first
...
## 14. Future execution is a separate capability
